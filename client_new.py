...
class WebSocketClient:
    ...
    async def execute_command(self, command: Command):
        ...
        if command.response_type == ResponseType.JSON_RESPONSE:
            response = await loop.run_in_executor(
                self.executor, self.executor_function, command.text
            )
            await self.send_text_response(
                RecieveTextResponse(
                    command_id=command.id,
                    response=CommandExecutionResponse(response=response),
    ...
    def executor_function(self, command_data: str) -> str:
        print(f"Executing command: {command_data}")
        return command_data  # Simulate command execution and return response
